...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
	{
		...
		int  algorithm = regular; //выбранный алгоритм движения
		...
		if( target_set && position_set )
			...
			currentAction = getAction(current_x, current_y, current_yaw, target_x, target_y);
			
			//взависимости от полученного действия рассчитаем упраляющее воздействие на шасси 			
			control = controlAction(algorithm, currentAction);
